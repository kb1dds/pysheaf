...
if np.allclose(np.dot(tl.cells[6].cofaces[1].corestriction,sec[0:3]),np.dot(tl.cells[7].cofaces[1].corestriction,sec[3:6])) and np.allclose(np.dot(tl.cells[6].cofaces[2].corestriction,sec[0:3]),np.dot(tl.cells[8].cofaces[2].corestriction,sec[6:9])) and np.allclose(np.dot(tl.cells[7].cofaces[2].corestriction,sec[3:6]),np.dot(tl.cells[8].cofaces[1].corestriction,sec[6:9])):
    print "Test passed"
else:
    print "Corestriction to an edge is mismatched"
...
L1=-1j/wavenumber*np.log(np.dot(tl.cells[7].cofaces[2].corestriction,sec[3:6])[0,0]/sec[7])/ft2m
L2=1j/wavenumber*np.log(np.dot(tl.cells[6].cofaces[2].corestriction,sec[0:3])[0,1]/sec[8])/ft2m
L3=-1j/wavenumber*np.log(np.dot(tl.cells[6].cofaces[1].corestriction,sec[0:3])[0,0]/sec[4])/ft2m
