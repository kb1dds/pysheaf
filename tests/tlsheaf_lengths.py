...
for tl,k in zip(sheaves,wavenumbers):
    ...
    L1=np.real(-1j/k*np.log(np.dot(tl.cells[7].cofaces[2].corestriction,sec[3:6])[0,0]/sec[7]))
    L2=np.real(1j/k*np.log(np.dot(tl.cells[6].cofaces[2].corestriction,sec[0:3])[0,1]/sec[8]))
    L3=np.real(-1j/k*np.log(np.dot(tl.cells[6].cofaces[1].corestriction,sec[0:3])[0,0]/sec[4]))
